...
CONFIGS: dict[str, tuple[str, str, str, str]] = {
    "smoke_2d.yaml": ("2d", "2D smoke", "Fast check", "morphovoxel.train_2d"),
    "phase1_2d.yaml": ("2d", "2D training", "Full growth", "morphovoxel.train_2d"),
    "smoke_3d.yaml": ("3d", "3D smoke", "Fast voxel check", "morphovoxel.train_3d"),
    "phase2_3d.yaml": ("3d", "3D training", "Single morphology", "morphovoxel.train_3d"),
    "smoke_conditional.yaml": ("conditional", "Conditional smoke", "Four genomes", "morphovoxel.train_conditional"),
    "phase3_conditional.yaml": ("conditional", "Conditional training", "Shared 3D rule", "morphovoxel.train_conditional"),
    "phase4_regeneration_training.yaml": ("regeneration", "Damage training", "State-pool recovery", "morphovoxel.train_conditional"),
    "smoke_regeneration.yaml": ("regeneration", "Regeneration smoke", "One damage sweep", "morphovoxel.evaluate_regeneration"),
    "phase4_regeneration.yaml": ("regeneration", "Regeneration evaluation", "Full damage matrix", "morphovoxel.evaluate_regeneration"),
    "smoke_ecology.yaml": ("ecology", "Ecology smoke", "Two organisms", "morphovoxel.run_ecology"),
    "phase5_ecology.yaml": ("ecology", "Ecology run", "Resource competition", "morphovoxel.run_ecology"),
    "ecology_experiments.yaml": ("ecology", "Ecology matrix", "All scenarios", "scripts/run_ecology_experiment.py"),
    "full_experiment.yaml": ("experiments", "Full experiment", "All phases", "scripts/run_full_experiment.py"),
}
...
def _launch(server: "DashboardServer", payload: dict[str, Any]) -> dict[str, Any]:
    ...
    frame_value = payload.get("frame_every", 1)
    if isinstance(frame_value, bool) or not isinstance(frame_value, int):
        raise ValueError("frame_every must be an integer from 1 to 1000")
    frame_every = frame_value
    if not 1 <= frame_every <= 1000:
        raise ValueError("frame_every must be an integer from 1 to 1000")
    overrides = {"device": device, "live_preview": live_preview, "frame_every": frame_every}
    ...
    with server.jobs_lock:
        ...
        run_name = requested_name or f"ui_{Path(filename).stem}_{job_id[-13:]}"
        ...
        try:
            if route.path == "/":
                ...
                with self.server.lab_lock:
                    ...
                    self._send(self.server.lab.frame_png(view, layer), "image/png")
...
function loadConfig(){const c=state.configs.find(c=>c.name===$('#configSelect').value);if(!c){$('#editor').value='';return}$('#editor').value=c.content;$('#editor').dataset.config=c.name;$('#kindLabel').textContent=c.kind.toUpperCase();const smoke=c.name.startsWith('smoke_'),requested=(c.content.match(/^\s*device:\s*(auto|cpu|cuda)\s*$/m)||[])[1]||'auto';$('#device').value=requested;$('#frameEvery').value=smoke?1:4;$('#frameEveryValue').value=$('#frameEvery').value;renderSliders();$('#warning').textContent=smoke?'Smoke presets validate the pipeline; they default to CPU because GPU launch overhead can dominate tiny runs.':c.name==='full_experiment.yaml'?'Full experiments can run for hours. You can stop the job from the jobs panel.':''}
...
function labView(){return lab?.dimensions===3?$('#labView').value:'plane'}function labLayer(){return lab?.dimensions===3?Number($('#labLayer').value):0}
function syncLabLayer(center=false){if(!lab||lab.dimensions!==3)return;const count=lab.layers[labView()]||1,input=$('#labLayer');input.max=count-1;if(center||Number(input.value)>=count)input.value=Math.floor(count/2);$('#labLayerValue').value=input.value}
async function drawLab(){if(!lab)return;const canvas=$('#labCanvas'),context=canvas.getContext('2d'),view=labView(),layer=labLayer(),version=lab.frame_version,image=new Image();await new Promise((resolve,reject)=>{image.onload=resolve;image.onerror=()=>reject(new Error('Could not render lab frame'));image.src=`/api/lab/frame?view=${encodeURIComponent(view)}&layer=${layer}&v=${version}`});if(!lab||lab.frame_version!==version||view!==labView()||layer!==labLayer())return;canvas.width=image.naturalWidth;canvas.height=image.naturalHeight;context.imageSmoothingEnabled=false;context.drawImage(image,0,0)}
async function setLab(meta){const changed=!lab||lab.run!==meta.run||lab.dimensions!==meta.dimensions;lab=meta;$('#labStatus').textContent=`${meta.run} · ${meta.dimensions}D`;$('#labStats').innerHTML=`<div class="lab-stat"><b>${meta.steps}</b><small>step</small></div><div class="lab-stat"><b>${Number(meta.steps_per_second).toFixed(1)}</b><small>steps/s</small></div><div class="lab-stat"><b>${meta.occupied_cells}</b><small>cells</small></div><div class="lab-stat"><b>${esc(meta.device)}</b><small>device</small></div>`;['#labPlay','#labStep','#labReset','#labClear'].forEach(id=>$(id).disabled=false);$('#labGenomeField').classList.toggle('lab-hidden',!meta.conditional);$('#lab3dControls').classList.toggle('lab-hidden',meta.dimensions!==3);if(changed){$('#labGenome').innerHTML=meta.genomes.map((name,index)=>`<option value="${index}">${esc(name)}</option>`).join('');$('#labGenome').value=meta.genome;$('#labView').value='slice';syncLabLayer(true)}else{$('#labGenome').value=meta.genome;syncLabLayer()}await drawLab()}
function pauseLab(){labPlaying=false;labPlayGeneration++;$('#labPlay').textContent='Play'}
...
async function editLab(payload,forceSeed=false,wait=true){if(!lab)return;await labAction(forceSeed?'seed':$('#labTool').value,payload,wait)}
...
function showRun(name){selectedRun=name;const r=state.runs.find(r=>r.name===name);if(!r)return;$('#resultTitle').textContent=r.name;$('#resultKind').textContent=r.kind.toUpperCase();$('#resultDate').textContent=new Date(r.updated).toLocaleString();let html=r.lab_ready?`<div class="actions" style="margin-bottom:14px"><button class="btn primary" data-open-lab="${esc(r.name)}">Open in Design Lab</button></div>`:'';html+=r.media.length?`<div class="gallery">${r.media.map(m=>`<figure class="media"><img src="${m.url}" alt="${esc(m.name)} visualization"><figcaption>${esc(m.name)}</figcaption></figure>`).join('')}</div>`:'<div class="empty">This run has no saved images yet.</div>';for(const metric of r.metrics){html+=`<div class="table-wrap"><table><caption style="padding:10px;text-align:left">${esc(metric.name)}</caption><thead><tr>${metric.columns.map(c=>`<th>${esc(c)}</th>`).join('')}</tr></thead><tbody>${metric.rows.map(row=>`<tr>${metric.columns.map(c=>`<td>${esc(row[c])}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`}if(r.logs)html+=`<h3>Training log</h3><pre>${esc(r.logs)}</pre>`;$('#resultBody').innerHTML=html;document.querySelectorAll('[data-open-lab]').forEach(button=>button.onclick=()=>openLab(button.dataset.openLab));runs()}
async function refresh(){try{const first=!state.configs.length;state=await api('/api/state');const h=state.hardware||{};$('#deviceStatus').textContent=h.cuda_available?`GPU · ${h.cuda_name}`:`Auto · ${h.auto_device||'CPU'}`;for(const select of [$('#device'),$('#labDevice')])select.querySelector('option[value="cuda"]').disabled=!h.cuda_available;$('#hardwareNote').textContent=(h.cuda_available?`Auto will use ${h.cuda_name}. Tiny smoke runs can still be faster on CPU.`:`Auto is using CPU because this PyTorch ${h.torch_version||''} build cannot access CUDA.`)+' Every step is smoothest; use 4+ to reduce preview I/O.';if(selectedRun&&!state.runs.some(r=>r.name===selectedRun))selectedRun=null;configs(first);syncLabRuns();runs();if(state.lab&&(!lab||state.lab.run!==lab.run||state.lab.frame_version!==lab.frame_version))setLab(state.lab);if(selectedRun)showRun(selectedRun)}catch(e){toast(e.message,true)}}
async function launch(){try{const job=await api('/api/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:TOKEN,config:$('#configSelect').value,content:$('#editor').value,run_name:$('#runName').value,device:$('#device').value,live_preview:$('#livePreview').checked,frame_every:Number($('#frameEvery').value)})});toast(`Started ${job.run_name}`);$('#runName').value='';await refresh()}catch(e){toast(e.message,true)}}async function stopJob(id){try{await api('/api/stop',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:TOKEN,job:id})});toast('Stop requested');await refresh()}catch(e){toast(e.message,true)}}
document.querySelectorAll('.nav button[data-kind]').forEach(b=>b.onclick=()=>{document.querySelectorAll('.nav button').forEach(x=>x.classList.remove('active'));b.classList.add('active');kind=b.dataset.kind;configs(true);runs()});$('#labNav').onclick=()=>$('#designLab').scrollIntoView({behavior:'smooth',block:'start'});$('#configSelect').onchange=loadConfig;$('#editor').onchange=renderSliders;$('#frameEvery').oninput=()=>$('#frameEveryValue').value=$('#frameEvery').value;$('#reset').onclick=loadConfig;$('#launch').onclick=launch;$('#refresh').onclick=refresh;$('#search').oninput=runs;$('#labLoad').onclick=()=>openLab();$('#labPlay').onclick=toggleLab;$('#labStep').onclick=()=>labControl('advance',{steps:Number($('#labSteps').value)});$('#labReset').onclick=()=>labControl('reset');$('#labClear').onclick=()=>labControl('clear');$('#labGenome').onchange=()=>labControl('genome',{genome:Number($('#labGenome').value)});$('#labView').onchange=()=>{syncLabLayer(true);drawLab()};$('#labLayer').oninput=()=>{$('#labLayerValue').value=$('#labLayer').value;drawLab()};$('#labSteps').oninput=()=>$('#labStepsValue').value=$('#labSteps').value;$('#labRadius').oninput=()=>$('#labRadiusValue').value=$('#labRadius').value;const labCanvas=$('#labCanvas');labCanvas.onpointerdown=event=>{if(event.button!==0||!lab)return;labEditing=true;labMoved=false;labCanvas.setPointerCapture(event.pointerId)};labCanvas.onpointermove=event=>{if(labEditing&&$('#labTool').value==='erase'){labMoved=true;clearTimeout(labClickTimer);editLab(labEditPayload(event),false,false)}};labCanvas.onpointerup=event=>{if(!labEditing)return;labEditing=false;const payload=labEditPayload(event);if(labMoved){labAction('erase',payload,true);return}if($('#labTool').value==='seed')editLab(payload);else{clearTimeout(labClickTimer);labClickTimer=setTimeout(()=>editLab(payload),450)}};labCanvas.onpointercancel=()=>labEditing=false;labCanvas.ondblclick=event=>{event.preventDefault();clearTimeout(labClickTimer);editLab(labEditPayload(event),true)};labCanvas.oncontextmenu=event=>event.preventDefault();refresh();setInterval(()=>{if(state.jobs.some(j=>j.status==='running'))refresh()},2000);
</script></body></html>'''
