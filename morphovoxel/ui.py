...
CONFIGS: dict[str, tuple[str, str, str, str]] = {
    ...
    "tree_family.yaml": ("tree", "Tree genome family", "Continuous inherited tree controls", "morphovoxel.train_conditional"),
    "tree_regeneration.yaml": ("tree", "Tree regeneration", "Persistent family with damage training", "morphovoxel.train_conditional"),
    ...
    "smoke_tree_family.yaml": ("tree", "Tree family smoke", "Fast continuous-genome check", "morphovoxel.train_conditional"),
    "smoke_tree_regeneration.yaml": ("tree", "Tree regeneration smoke", "Fast damage-pipeline check", "morphovoxel.train_conditional"),
...
def _run_kind(run: Path) -> str:
    ...
    if model_kind == "tree_family":
        return "environment" if "environment" in run.name.lower() else "tree"
...
$('.brand small').textContent='Organism workbench';$('.top h1').textContent='Organism design workbench';const trainingPanel=document.querySelector('.grid>.panel');trainingPanel.id='trainingLab';trainingPanel.querySelector('h2').textContent='Train an organism';$('.nav').setAttribute('aria-label','Workbench areas');$('.nav').innerHTML='<button class="active" data-kind="all">Overview</button><button data-kind="specialist">Specialist Lab</button><button id="treeLabNav">Tree Genome Lab</button><button id="environmentLabNav">Environment Lab</button><button id="archiveNav">Variant Archive</button><button id="labNav">View Checkpoints</button><button data-kind="ecology">Ecology</button><button data-kind="legacy">Legacy</button>';
$('#labRun').closest('.field').insertAdjacentHTML('afterend',`<label class="field"><span>Checkpoint</span><select id="labCheckpoint" aria-label="Checkpoint file"></select></label><label class="field"><span>Display</span><select id="labDisplay"><option value="organism">Organism</option><option value="target">Training target</option></select></label><div class="model-summary" id="labModelSummary"><span>Model</span><b>Open a checkpoint</b></div>`);
...
function matchesArea(item){if(kind==='all')return true;if(kind==='legacy')return !['specialist','tree','environment'].includes(item.kind)&&!item.name.startsWith('tree_')&&!item.name.startsWith('smoke_tree_');return item.kind===kind}
...
function syncEnvironmentOverlays(meta){const select=$('#labDisplay'),previous=select.value;select.querySelectorAll('[data-environment-overlay]').forEach(option=>option.remove());for(const name of meta.environment_overlays||[])select.insertAdjacentHTML('beforeend',`<option data-environment-overlay value="environment:${esc(name)}">Overlay · ${esc(prettyName(name))}</option>`);if([...select.options].some(option=>option.value===previous))select.value=previous;else select.value='organism'}
...
async function setLab(meta){await loadTreeSchema();const changed=!lab||lab.run!==meta.run||lab.checkpoint!==meta.checkpoint||lab.model_kind!==meta.model_kind||lab.dimensions!==meta.dimensions;if(changed){labDeviceRate=0;treeDirty=false;environmentDirty=false}lab=meta;if(Number(meta.steps_per_second)>0)labDeviceRate=Number(meta.steps_per_second);syncLabSpeed();$('#labStatus').textContent=`${meta.run} · ${meta.dimensions}D`;['#labPlay','#labStep','#labReset','#labClear'].forEach(id=>$(id).disabled=false);$('#treeValidate').disabled=!meta.active_tree_genome||labValidating;$('#labGenomeField').classList.toggle('lab-hidden',!meta.conditional||meta.continuous_genome);$('#lab3dControls').classList.toggle('lab-hidden',meta.dimensions!==3);$('#labDisplay option[value="target"]').textContent=`Training target · ${meta.target_name}`;$('#labRun').value=meta.run;syncLabCheckpoints();$('#labCheckpoint').value=meta.checkpoint;if(changed){$('#labDisplay').value='organism';$('#labGenome').innerHTML=(meta.genomes||[]).map((name,index)=>`<option value="${index}">${esc(name)}</option>`).join('');$('#labGenome').value=meta.genome;$('#labView').value=meta.dimensions===3?'voxels':'slice';labYaw=-.7;labPitch=.55;labZoom=1;syncLabLayer(true)}else{$('#labGenome').value=meta.genome;syncLabLayer()}syncTreePanels(meta,changed);renderLabStats();await drawLab()}
