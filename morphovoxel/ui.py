...
async function setLab(meta){const changed=!lab||lab.run!==meta.run||lab.dimensions!==meta.dimensions;if(changed)labDeviceRate=0;lab=meta;if(Number(meta.steps_per_second)>0)labDeviceRate=Number(meta.steps_per_second);syncLabSpeed();$('#labStatus').textContent=`${meta.run} · ${meta.dimensions}D`;$('#labStats').innerHTML=`<div class="lab-stat"><b>${meta.steps}</b><small>step</small></div><div class="lab-stat"><b>${Number(meta.steps_per_second).toFixed(1)}</b><small>device steps/s</small></div><div class="lab-stat"><b>${meta.occupied_cells}</b><small>cells</small></div><div class="lab-stat"><b>${esc(meta.device)}</b><small>device</small></div>`;['#labPlay','#labStep','#labReset','#labClear'].forEach(id=>$(id).disabled=false);$('#labGenomeField').classList.toggle('lab-hidden',!meta.conditional);$('#lab3dControls').classList.toggle('lab-hidden',meta.dimensions!==3);if(changed){$('#labGenome').innerHTML=meta.genomes.map((name,index)=>`<option value="${index}">${esc(name)}</option>`).join('');$('#labGenome').value=meta.genome;$('#labView').value=meta.dimensions===3?'voxels':'slice';labYaw=-.7;labPitch=.55;labZoom=1;syncLabLayer(true)}else{$('#labGenome').value=meta.genome;syncLabLayer()}await drawLab()}
