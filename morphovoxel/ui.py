        ...
        try:
            if route.path == "/":
                ...
                with self.server.lab_lock:
                    if self.server.lab is None:
                        raise ValueError("open a checkpoint-backed run in the lab first")
                    self._json(self.server.lab.voxel_data())
...
let TOKEN='__TOKEN__';let state={configs:[],runs:[],jobs:[],lab:null},kind='all',selectedRun=null,lab=null,labPlaying=false,labBusy=false,labLoading=false,labEditing=false,labMoved=false,labPlayGeneration=0,labClickTimer=null,labVoxelData=null,labYaw=-.7,labPitch=.55,labZoom=1,labOrbiting=false,labLastX=0,labLastY=0;const $=s=>document.querySelector(s),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const voxelOption=document.createElement('option');voxelOption.value='voxels';voxelOption.textContent='3D voxels';$('#labView').append(voxelOption);
...
function labView(){return lab?.dimensions===3?$('#labView').value:'plane'}function labVoxelMode(){return lab?.dimensions===3&&labView()==='voxels'}function labLayer(){return lab?.dimensions===3&&!labVoxelMode()?Number($('#labLayer').value):0}
function syncLabLayer(center=false){if(!lab||lab.dimensions!==3)return;const input=$('#labLayer'),voxel=labVoxelMode();input.disabled=voxel;$('#labTool').disabled=voxel;if(voxel){$('#labLayerValue').value='—';return}const count=lab.layers[labView()]||1;input.max=count-1;if(center||Number(input.value)>=count)input.value=Math.floor(count/2);$('#labLayerValue').value=input.value}
