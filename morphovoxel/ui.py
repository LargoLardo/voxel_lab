    ...
    def do_POST(self) -> None:  # noqa: N802
        try:
            ...
            if self.path == "/api/run":
                ...
                path = find_checkpoint(run, checkpoint)
                if path is None:
                ...
                with self.server.jobs_lock:
                    if any(
                        ...
                        raise ValueError("cannot delete a checkpoint while its run is active")
                with self.server.lab_load_lock:
                    with self.server.lab_lock:
                        closed_lab = bool(
                            self.server.lab
                            and self.server.lab.run_name == run_name
                            and self.server.lab.checkpoint_name == checkpoint
                        )
                        path.unlink()
                        if closed_lab:
                            self.server.lab = None
                self._json({"deleted": checkpoint, "run": run_name, "closed_lab": closed_lab})
                ...
                if device not in {"auto", "cpu", "cuda"}:
                    raise ValueError("device must be auto, cpu, or cuda")
                scope = str(payload.get("scope", "specimen"))
                if scope not in {"specimen", "family"}:
                    raise ValueError("scope must be specimen or family")
                horizons = payload.get("horizons", [1024, 2048])
                if (
                    not isinstance(horizons, list) or not 1 <= len(horizons) <= 2
                    or any(isinstance(value, bool) or not isinstance(value, int) or not 512 <= value <= 2048 for value in horizons)
                    or len(set(horizons)) != len(horizons)
                ):
                    raise ValueError("horizons must contain one or two distinct integers from 512 through 2048")
                recovery_steps = payload.get("recovery_steps", 256)
                if isinstance(recovery_steps, bool) or not isinstance(recovery_steps, int) or not 128 <= recovery_steps <= 1024:
                    raise ValueError("recovery_steps must be an integer from 128 through 1024")
                fire_seeds = payload.get("fire_seeds", [1001, 1002, 1003, 1004])
                if (
                ...
                if not self.server.validation_lock.acquire(blocking=False):
                    raise ValueError("checkpoint evaluation is already running")
                try:
                    with self.server.lab_load_lock:
                        session = LabSession.from_run(run, device, checkpoint)
                    reports = [
                        session.validate_tree_candidate(
                            steps=horizon,
                            recovery_steps=recovery_steps,
                            fire_seeds=tuple(fire_seeds),
                            scope=scope,
                        )
                ...
                report_values = [value["report"] for value in reports]
                self._json({
                    "run": run_name,
                    ...
                    "accepted": all(bool(value["accepted"]) for value in report_values),
                    "worst_score": min(float(value["worst_score"]) for value in report_values),
                    "reports": reports,
                })
...
$('#variantArchive').insertAdjacentHTML('beforebegin',`<section class="panel lab-panel" id="stabilityEvaluation"><div class="panel-head"><div><div class="eyebrow">LONG-HORIZON TEST</div><h2>Stability Evaluation</h2></div><span class="status" id="evaluationStatus">READY</span></div><div class="panel-body"><div class="evaluation-controls"><label class="field"><span>Checkpoint run</span><select id="evaluationRun"></select></label><label class="field"><span>Checkpoint</span><select id="evaluationCheckpoint"></select></label><label class="field"><span>Compute device</span><select id="evaluationDevice"><option value="auto">Auto</option><option value="cpu">CPU</option><option value="cuda">CUDA GPU</option></select></label><label class="field"><span>Evaluation scope</span><select id="evaluationScope"><option value="specimen">Default specimen</option><option value="family">Full genome family panel</option></select></label><label class="field"><span>Undamaged horizons</span><select id="evaluationHorizons"><option value="1024,2048">1,024 and 2,048 steps</option><option value="1024">1,024 steps</option><option value="2048">2,048 steps</option></select></label><label class="field"><span>Recovery after damage</span><select id="evaluationRecovery"><option value="256">256 steps</option><option value="128">128 steps</option><option value="512">512 steps</option></select></label><label class="field evaluation-seeds"><span>Fire-mask / damage seeds</span><input id="evaluationSeeds" value="1001, 1002, 1003, 1004"></label><button class="btn primary" id="evaluationRunButton">Evaluate checkpoint</button></div><div class="hint evaluation-note">The two-horizon test damages the organism at different ages. Seeds vary stochastic updates and cycle through sphere, cuboid, top, and randomized-dropout damage. Family-panel evaluation can take several minutes.</div><div id="evaluationResults"><div class="empty">Choose a checkpoint, then run the long-horizon stability test.</div></div></div></section>`);
...
function clearLabView(){pauseLab();lab=null;labVoxelData=null;labTargetVoxelData=null;labTargetKey='';$('#labStatus').textContent='NO RUN';$('#labModelSummary').innerHTML='<span>Model</span><b>Open a checkpoint</b>';for(const id of ['#labPlay','#labStep','#labReset','#labClear','#openTreeGenomeWindow','#openEnvironmentWindow'])$(id).disabled=true;for(const id of ['#labCanvas','#labTargetCanvas']){const canvas=$(id);canvas.getContext('2d').clearRect(0,0,canvas.width,canvas.height)}}
async function deleteLabCheckpoint(){const run=$('#labRun').value,checkpoint=$('#labCheckpoint').value;if(!run||!checkpoint)return;if(!confirm(`Permanently delete ${run} / ${checkpoint}?\n\nPipeline presets that reference this file will stop working until you choose another checkpoint.`))return;const button=$('#labDeleteCheckpoint');button.disabled=true;try{const result=await labPost('/api/checkpoint/delete',{run,checkpoint});if(result.closed_lab)clearLabView();toast(`Deleted ${run} / ${checkpoint}`);await refresh()}catch(error){toast(error.message,true)}finally{syncLabCheckpoints()}}
